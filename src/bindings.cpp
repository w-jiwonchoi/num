...
NB_MODULE(kokkos_spmv, m) {
    ...
    m.doc() = R"(
kokkos_spmv — Multi-GPU Sparse Linear Algebra Library (v0.2)
============================================================
...
GPU arrays are accepted zero-copy from CuPy / PyTorch / JAX.
    ...
    "Pass device_id = rank % n_gpu for multi-GPU.");
    ...
    m.def("cuda_aware_mpi", []() {
        ...
        return false;
#endif
    }, "True if the library was compiled with -DENABLE_CUDA_AWARE_MPI=ON.");
    ...
    m.def("upload_csr",
           ...
           int nrows, int ncols) -> CrsMatrixHandle
        {
            return CrsMatrixHandle(
                values_nb.data(), colind_nb.data(), rowptr_nb.data(),
                nrows, ncols);
        },
    ...
    m.def("spmv",
        [](const CrsMatrixHandle& A,
           f64_1d_gpuc x_nb, f64_1d_gpu y_nb, bool use_kokkoskernels)
        {
    ...
    m.def("dist_spmv",
        [](const DistCrsHandle& A_dist,
           f64_1d_gpuc x_nb, f64_1d_gpu y_nb,
        ...
        R"(
Distributed SpMV: y_local = (A x)_local via halo exchange.

kokkoskernels : local kernel = KokkosKernels/cuSPARSE (True)
                or custom hierarchical TeamPolicy (False)
overlap       : overlap interior SpMV with halo communication
)");

    m.def("cg_solve",
        [](const DistCrsHandle& A_dist,
           f64_1d_gpuc b_nb, f64_1d_gpu x_nb,
